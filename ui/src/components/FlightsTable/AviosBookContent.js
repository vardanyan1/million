...
const BookWindowContent = ({points}) => {
    ...
    return (
        <>
            ...
            <Text mb={4}>
                {t('howToBookAvios.subheader')}
            </Text>
            <OrderedList fontWeight={"semibold"} fontSize={"sm"}>
                <ListItem mb={4}>
                    {t('howToBookAvios.bookItem1')}
                </ListItem>
                <ListItem mb={4}>
                    {t('howToBookAvios.bookItem2')}
                </ListItem>
                <ListItem mb={4}>
                    {t('howToBookAvios.bookItem3')}
                </ListItem>
            </OrderedList>
            <Divider/>
            <Flex justify={"space-between"} py={4}>
                ...
                <Box>
                    <Text fontSize={"md"} fontWeight={"bold"}>{numberFormat.format(points)} {t('table.avios')}</Text>
            ...
            <Button as="a" w={'100%'} backgroundColor="#DD0000" color="white" 
                ...
                target='_blank' href={howToBookAviosLink}>
                {t('table.bookButton')}
            </Button>
        </>
    )
