...
sender_email = 'noreply@rewardflights.io'
password = 'wcoqxgcqricpuxnp'

# Authentication credentials
# use the sign-in credentials of your account
auth_email = "jason@thorndyke.ai"
auth_pass = "MillionPoints+"#coqHeh-sazduv-3hitwi"

BASE_URL = "https://rewardflights.io/"
...
def extract_flight_details_for_alert(notification):
    ...
    #print(notification)
    #print(notification['Departure Air'])
    ...
    try:
        ...
        flight_data = filter_excluded_carriers(flight_data, notification['Departure Air'], notification['Destination Air'])
        ...
        #print(f"flight_data['First Departure Date'], {flight_data['First Departure Date']}")
        #print(f"notification['Start Date'], {notification['Start Date']}")
        ...
        #print(f"matching_flights, {matching_flights}")
        for idx, flight_row in matching_flights.iterrows():
            result_row = copy.deepcopy(notification)  # Start with all columns from Alert_user.txt
            #print(f"flight_data.columns, {flight_data.columns}")
            #print(f"notification.columns, {notification.keys()}")
        ...
        pass
    #print(f"DataFrame(results), {pd.DataFrame(results)}")
...
def filter_excluded_routes(flight_data, departure, arrival):
    ...
    if exclusion_routes and isinstance(exclusion_routes[0], str):
...
def filter_excluded_carriers(flight_data, departure, arrival):
    # Fetch the exclusion routes for the specific departure and arrival
    exclusion_carriers = exclusions_df[
        (exclusions_df["Departure"] == departure) &
        (exclusions_df["Arrival"] == arrival)
    ]["Carrier Exclusions"].tolist()

    # If there are any exclusion routes for this departure and arrival
    if exclusion_carriers and isinstance(exclusion_carriers[0], str):
        # Convert the string of routes into a list of routes
        excluded_carriers = exclusion_carriers[0].split(', ')
        # print(f"excluded_carriers {excluded_carriers}")
        # Filter out the flights with excluded routes
        for carrier in excluded_carriers:
            flight_data = flight_data[~flight_data['Aircraft Details'].str.contains(carrier, na=False)]
...
def send_mail(receiver_email, flight_details, sender_email, password):
    ...
    #print(f"flight_details['First Departure Date'], {flight_details['First Departure Date']}")
    ...
    #print(f"flight_details['Formatted Date'], {flight_details['Formatted Date']}")
    ...
    info_availability = 'Note - availability changes quickly! View these available flights on <a href="https://rewardflights.io/">Reward Flights</a> and see if the itinerary works for your travel plans.'
    ...
    website_link = "©2023 rewardflights.io"

    body = f"{greeting}<br><br>{intro_text}<br><br>{route}<br>{travel_dates}<br>{selected_classes}<br><br>{outbound_dates_text}<br><br>{info_availability}<br><br>{beaware}<br><br>{website_link}"
...
def send_flight_details_to_users(token, sender_email, password):
    ...
    for notification in notifications:
        ...
        #print(f"flight_details_df, {flight_details_df}")
